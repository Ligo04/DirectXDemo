...
class GerstnerWavesEffect:public IEffect,public IEffectTransform,public IEffectTextureDiffuse,public IEffectDisplacementMap
{
public:
	struct GerstnerWaveParameter
	{
		GerstnerWaveParameter() = default;

		GerstnerWaveParameter(const GerstnerWaveParameter&) = default;
		GerstnerWaveParameter& operator=(const GerstnerWaveParameter&) = default;

		GerstnerWaveParameter(GerstnerWaveParameter&&) = default;
		GerstnerWaveParameter& operator=(GerstnerWaveParameter&&) = default;

		GerstnerWaveParameter(const float& _wavelength, const float& _amplitude, const float& _wavespeed, const float& _direction)
			:waveLength(_wavelength), amplitude(_amplitude), wavespeed(_wavespeed), direction(_direction) {}

		float waveLength;				// 波长
		float amplitude;				// 振幅
		float wavespeed;				// 波速
		float direction;				// 方向(角度)
	};

public:
	GerstnerWavesEffect();
	virtual ~GerstnerWavesEffect() override;

	GerstnerWavesEffect(GerstnerWavesEffect&& moveFrom) noexcept;
	GerstnerWavesEffect& operator=(GerstnerWavesEffect&& moveFrom) noexcept;

	// 获取单例
	static GerstnerWavesEffect& Get();
	...
	// 设置是否开启Gpu绘制
	void SetEnableGpu(bool isEnable);

	//是否开启线框状态
	void SetRSWireframe(bool isWireframe);
	...
	void SetTextureNormalMap(ID3D11ShaderResourceView* textureNormalMap) override;
	// 设置位移贴图
	void SetTextureDisplacementMap(ID3D11ShaderResourceView* textureDisplacementMap);

	// 设置计算后的法线SRV
	void SetTextureNormalSRV(ID3D11ShaderResourceView* textureNormalSrv);

	// 设置计算后的顶点数据SRV
	void SetTexturePositionSRV(ID3D11ShaderResourceView* texturepositionSrv);


	// 设置顶点输出UAV
	void SetTexturePositonUAV(ID3D11UnorderedAccessView* texturePositonUav);

	// 设置法线输出UAV
	void SetTextureNormalUAV(ID3D11UnorderedAccessView* textureNormalUav);
	...
	// 设置波浪数目
	void SetNumWaves(UINT numwaves);
	...
	// 设置参数
	void SetGpuGerStnerWave(UINT pos,const GerstnerWaveParameter& parameter);

	// 设置列数
	void SetNumCols(UINT numcols);
